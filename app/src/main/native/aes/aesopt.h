/*
 ...
 AES bit sequence indexes map to higher numeric significance within bytes.
  ...
  struct encrypt_ctx  (structure for the cipher encryption context)
  struct decrypt_ctx  (structure for the cipher decryption context)
  AES_RETURN                the function return type

  C subroutine calls:

  AES_RETURN aes_encrypt_key128(const unsigned char *key, encrypt_ctx cx[1]);
  AES_RETURN aes_encrypt_key192(const unsigned char *key, encrypt_ctx cx[1]);
  AES_RETURN aes_encrypt_key256(const unsigned char *key, encrypt_ctx cx[1]);
  AES_RETURN aes_encrypt(const unsigned char *in, unsigned char *out,
                                                  const encrypt_ctx cx[1]);

  AES_RETURN aes_decrypt_key128(const unsigned char *key, decrypt_ctx cx[1]);
  AES_RETURN aes_decrypt_key192(const unsigned char *key, decrypt_ctx cx[1]);
  AES_RETURN aes_decrypt_key256(const unsigned char *key, decrypt_ctx cx[1]);
  AES_RETURN aes_decrypt(const unsigned char *in, unsigned char *out,
                                                  const decrypt_ctx cx[1]);
